<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Resignations.aspx.cs" Inherits="Human_Resource.Views.Employees.Resignations" %>
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
        ...
        function getEmpInfo(sel) {
            ...
            if (id == "" || id==0) {
                ...
                $('[id*=btn_reject]').prop('disabled', true);
            }
            else
                $('[id*=btn_save]').prop('disabled', false);
            $('[id*=btn_reject]').prop('disabled', false);
            ...
            $.ajax({
                type: "POST",
                url: "Resignations.aspx/GetEmpInfo",
                ...
                success: function (data) {
                    for (var prop in data) {
                        ...
                        $('[id*=hdn_rewardB]').val(item.Reward);
                        $('[id*=txt_reason]').val(item.Reason);
        ...
        function displayVacAccount() {
            ...
            if ($("[id*=chk_reward]").is(':checked')) {
                ...
                $("[id*=txt_reward]").html($("[id*=hdn_rewardB]").val());
                $("[id*=hdn_rewardValue]").val($("[id*=hdn_rewardB]").val());
            }
            else {
        ...
        function resetValues() {
            $('[id*=txt_dues]').val(0);
            $("[id*=txt_vacationsAccount]").html("0");
            $("[id*=txt_vacationsAccountValue]").val("0");
            $("[id*=txt_reward]").html("0");
            $("[id*=txt_rewardValue]").val("0");
            $("[id*=txt_total]").html("0");
            $("[id*=txt_reason]").val("0");
                   ...
                   <div class="col-12">
                        <div class="bcrumb-block">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item"><a id="a_title" href="#"><asp:Literal  runat="server" Text="<%$ Resources:Labels,Resignations%>" /></a></li>
                                </ol>
                            </nav>
                        </div>
                    </div>
                  ...
                  <div class="col-12">
                      <div class="c-form">
                          <div class="row">
                               <div class="form-group" style="display:block">
                                        ...
                                        <span><asp:Literal  runat="server" Text="<%$ Resources:Labels,Reason%>" /></span>
                                         <asp:TextBox type="text" class="content-textarea disabled" ID="txt_reason" name="txt_reason"  TextMode="MultiLine" Rows="10" runat="server" CssClass="width:80%" onchange="removeValidation($(this));"/>
                                        <div class="invalid-feedback"><asp:Literal  runat="server" Text="<%$ Resources:Labels,ValueIsRequired%>" /></div>
                          ...
                          <div class="row dismissal-table">
                              ...
                              <div class="col-4">
                                  ...
                                  <span id="txt_vacationsAccount" runat="server"><asp:Literal  runat="server"   Text="0"> </asp:Literal></span>
                                   ...
                                   <div class="row th">
                                        <input type="hidden" id="hdn_rewardB" name="hdn_rewardB" runat="server" />
                          ...
                          <div class="row">&nbsp;<br /><br /></div>
                            ...
                            <Button type="button" runat="server" class="btn btn-delete" id="btn_reject" disabled
                              onserverclick="btn_reject_Click" >
                                <asp:Literal  runat="server" Text=" <%$ Resources:Labels,Reject%>" />
                            </Button>  
                            <Button type="button" runat="server" class="btn  btn-new" id="btn_save" disabled
                              onserverclick="btn_save_Click" >
                                <asp:Literal  runat="server" Text=" <%$ Resources:Labels,Approve%>" />
                            </Button>
                </div>
                </div>
              </div>
